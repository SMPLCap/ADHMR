...
class InstaVariety_DPO(data.Dataset):
    def __init__(self, transform, data_split, multi_n=50):
        # super(PW3D_DPO, self).__init__(transform, data_split, multi_n=50)

        self.transform = transform

        self.img256_root_dir = '/mnt/AFS_shenwenhao/ScoreHypo/data/dpo/images/InstaVariety'
        self.annot_dir = '/mnt/AFS_shenwenhao/ScoreHypo/data/dpo/annotations'
        
        self.multi_n = multi_n

        self.use_cache = True               # always use cache
        self.db_dpo_path_cache = osp.join(cfg.data_dir, 'cache_dpo', f'InstaVariety_db_dpo_multin_{self.multi_n}.npz')
        self.dpo_id_path_cache = osp.join(cfg.data_dir, 'cache_dpo', f'InstaVariety_db_dpo_multin_{self.multi_n}_id.npz')
        ...
        if self.use_cache and osp.isfile(self.db_dpo_path_cache):
            ...
            self._dpo_ann_file = osp.join(self.annot_dir, f'new_InstaVariety_train_interval_1_multihypo_{multi_n}.npz')
    ...
    def __getitem__(self, idx):
        ...
        # transform score_joints to new coordinates
...
class Cache():
    ...
    def load(self, load_path):
        ...
        print(self.data_len, len(self.cache))  # data_len
