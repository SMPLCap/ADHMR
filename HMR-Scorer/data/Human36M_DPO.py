'''
Score ScoreHypo's predictions on Human36M.
'''
import os
...
class Human36M_DPO(data.Dataset):
    def __init__(self, transform, data_split, multi_n=50):
        # super(PW3D_DPO, self).__init__(transform, data_split, multi_n=50)

        self.transform = transform

        self.img256_root_dir = '/mnt/AFS_shenwenhao/ScoreHypo/data/dpo/images/H36MDataset'
        self.annot_dir = '/mnt/AFS_shenwenhao/ScoreHypo/data/dpo/annotations'
        
        self.multi_n = multi_n

        self.use_cache = False
        self.db_dpo_path_cache = osp.join(cfg.data_dir, 'cache_dpo', f'human36m_db_dpo_multin_{self.multi_n}.npz')
        self.dpo_id_path_cache = osp.join(cfg.data_dir, 'cache_dpo', f'human36m_db_dpo_multin_{self.multi_n}_id.npz')
        ...
        if self.use_cache and osp.isfile(self.db_dpo_path_cache):
            ...
            self._dpo_ann_file = osp.join(self.annot_dir, f'new_H36MDataset_train_interval_15_multihypo_{multi_n}.npz')
    ...
    def load_dpo_pt(self):
        ...
        for i, dpo_pair in enumerate(tqdm(dpo_db)):
            labels.append({
                'img_path': str(dpo_pair['img_path']),
    ...
    def __getitem__(self, idx):
        ...
        score_joints_2d = (score_joints_2d + 1) * 127.5  # 从 [-1, 1] 转到 [0, 255]
...
class Cache():
    ...
    def load(self, load_path):
        ...
        assert self.data_len == len(self.cache) - 1  # data_len
        self.cache = None
