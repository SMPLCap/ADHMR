...
from utils.preprocessing import load_img, process_bbox, augmentation, process_db_coord, process_human_model_output, \
    get_fitting_error_3D, augmentation_seg, process_db_coord_w_cam
...
class HumanDataset_Scorer_Test(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # TODO: temp solution, should be more general. But SHAPY is very special
            ...
            # process smplx_param_pred
            smplx_param_pred = data['smplx_param_pred']
            cam_trans_pred = np.array(smplx_param_pred['trans']) if 'trans' in smplx_param_pred else None
            if focal is not None and princpt is not None:
                if do_flip:
                    cam_trans_pred[0] = -cam_trans_pred[0]
            else:
                raise NotImplementedError
            # smplx coordinates and parameters
            smplx_joint_img_pred, smplx_joint_cam_pred, smplx_joint_trunc_pred, smplx_pose_pred, smplx_shape_pred, smplx_expr_pred, \
            smplx_pose_valid_pred, smplx_joint_valid_pred, smplx_expr_valid_pred, smplx_mesh_cam_orig_pred = process_human_model_output(
                smplx_param_pred, self.cam_param, do_flip, img_shape, img2bb_trans, rot, 'smplx',
                joint_img=None if self.cam_param else joint_img)
            ...
            gen_output = {'smplx_pose': smplx_pose_pred,
                          'smplx_shape': smplx_shape_pred,
                          'smplx_expr': smplx_expr_pred,
                          'cam_trans' : cam_trans_pred,
                          }
