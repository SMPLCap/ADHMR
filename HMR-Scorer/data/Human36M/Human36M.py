...
from utils.preprocessing import load_img, process_bbox, augmentation, process_db_coord, process_human_model_output, \
                                get_fitting_error_3D, process_human_model_output_clean
...
class Human36M_Clean(Human36M):
    def __init__(self, transform, data_split):
        ...
        # cfg.Human36M_train_sample_interval = 2
        self.annot_path_cache = osp.join(cfg.data_dir, 'cache_for_cleaning', f'Human36M_{self.data_split}.npz')
        if self.use_cache and osp.isfile(self.annot_path_cache):
            print(f'[{self.__class__.__name__}] loading cache from {self.annot_path_cache}')
            datalist = Cache(self.annot_path_cache)
            assert datalist.data_strategy == getattr(cfg, 'data_strategy', None), \
    ...
    def load_data(self):
        ...
        if getattr(cfg, 'data_strategy', None) == 'balance' and self.data_split == 'train':
            print(f'[Human36M] Using [balance] strategy with datalist NOT shuffled...')
            # random.shuffle(datalist)
    ...
    def __getitem__(self, idx):
        ...
        if self.data_split == 'train':
            ...
            smplx_joint_img, smplx_joint_cam, smplx_joint_trunc, smplx_pose, smplx_shape, smplx_expr, \
                smplx_pose_valid, smplx_joint_valid, smplx_expr_valid, smplx_mesh_cam_orig = \
                    process_human_model_output_clean(smplx_param, cam_param, do_flip, img_shape, img2bb_trans, rot, 'smplx')
