...
def augmentation(img, bbox, data_split):
    ...
    focal_scale = cfg.input_img_shape[1] / (bbox[2] * scale)
    img = np.clip(img * color_scale[None, None, :], 0, 255)
    return img, trans, inv_trans, rot, do_flip, focal_scale


def augmentation_seg(img, seg_img, bbox, data_split):
    if getattr(cfg, 'no_aug', False):
        scale, rot, color_scale, do_flip = 1.0, 0.0, np.array([1, 1, 1]), False
    elif data_split == 'train':
        scale, rot, color_scale, do_flip = get_aug_config()
    ...
    img, trans, inv_trans = generate_patch_image(img, bbox, scale, rot, do_flip, cfg.input_img_shape)
    seg_img, _, _ = generate_patch_image(seg_img, bbox, scale, rot, do_flip, cfg.input_img_shape)
    focal_scale = cfg.input_img_shape[1] / (bbox[2] * scale)
    img = np.clip(img * color_scale[None, None, :], 0, 255)
    return img, seg_img, trans, inv_trans, rot, do_flip, focal_scale
...
def process_db_coord_w_cam(joint_img, joint_cam, princpt, cam_trans, joint_valid, do_flip, img_shape, flip_pairs,
                            img2bb_trans, rot, src_joints_name, target_joints_name):
    joint_img_original = joint_img.copy()
    princpt_original = princpt.copy()
    joint_img, joint_cam, joint_valid, princpt = joint_img.copy(), joint_cam.copy(), joint_valid.copy(), princpt.copy()
    cam_trans_original = cam_trans.copy()
    ...
    # do affine transformation to princpt
    ...
    return joint_img, princpt, cam_trans, joint_cam_wo_ra, joint_cam_ra, joint_valid, joint_trunc, rot_aug_mat
...
def process_human_model_output_clean(human_model_param, cam_param, do_flip, img_shape, img2bb_trans, rot, human_model_type, joint_img=None):
    if human_model_type == 'smplx':
        human_model = smpl_x
        rotation_valid = np.ones((smpl_x.orig_joint_num), dtype=np.float32)
        coord_valid = np.ones((smpl_x.joint_num), dtype=np.float32)
