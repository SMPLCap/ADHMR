...
def get_twist(rot_mats, joints, parents):
    ...
    # angle_twist = []
    angle_twist_all = []
    for k in range(0, rot_mats.shape[0]):
        # error = False
        angle_twist = []
        for i in range(1, rot_mats.shape[1]):
            ...
            # rot = rot_mats[:, i]
            u = rel_joints[k, childs[i]][None, ...]
            rot = rot_mats[k, i][None, ...]
        ...
        angle_twist_ret = torch.stack(angle_twist, dim=1)
        
        angle_twist_all.append(angle_twist_ret)
        
    angle_twist_all = torch.cat(angle_twist_all, dim=0)

    # return angle_twist
    return angle_twist_all
