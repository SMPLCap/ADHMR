...
class HumanDataset(torch.utils.data.Dataset):
    ...
    def load_data(self, train_sample_interval=1, test_sample_interval=1):
        ...
        for i in tqdm.tqdm(range(int(num_examples))):
            ...
            datalist.append({
                ...
                'width': np.int64(img_shape[1]),
                'height': np.int64(img_shape[0]),
                ...
                # 'twist_phi': phi, #3
                # 'twist_weight': phi_weight,
                'f': f,                                           # [DEBUG] use meta focal_length in content
                'c': c,                                           # [DEBUG] use meta principal_point in content
    ...
    def __getitem__(self, idx):
        ...
        if self.data_split == 'train':
            ...
            if not dummy_cord: 
                joint_img[:, 2] = (joint_img[:, 2] / (self._cfg.body_3d_size / 2) + 1) / 2. * self._cfg.output_hm_shape[0]  # discretize depth
        ...
        # TODO: temp solution, should be more general. But SHAPY is very special
            ...
            inputs = {'img': img}
            if self._cfg.shapy_eval_split == 'val':
